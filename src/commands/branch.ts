...
const edit = new Command("edit")
  ...
  .option("--base <branch>", "Base branch for comparison", undefined)
  .action(async (subCommandOpts: Partial<BranchCommandOptions>, command: Command) => {
    // Get options from parent and globals separately
    const parentAndGlobalOpts = command.optsWithGlobals();
    
    // Pass options distinctly to analyzeBranch
    await analyzeBranch({ 
      options: parentAndGlobalOpts, // Parent + Globals
      subCommandOptions: subCommandOpts // Subcommand specifics (like base)
    });
  });
...
export const branchCommand = new Command("branch")
  ...
  .option("--name <branch>, --branch <branch>", "Branch name (defaults to current)");
